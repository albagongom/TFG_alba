...
# Ejecutamos el script Python leer_json.py en cada archivo .json en la carpeta de videos
for json_file in "$CURRENT_DIR"/*.json; do
  if [ -f "$json_file" ]; then
    # Ejecutamos el script Python leer_json.py en el archivo .json
    python3 leer_json_speaker.py "$json_file"
    echo "Procesado con leer_json_speaker.py: $json_file"
    ...
    if [ -f "$output_txt" ]; then
      ...
      echo "Movido: $output_txt -> $TRANSCRIPCIONES_DIR/"
    else
