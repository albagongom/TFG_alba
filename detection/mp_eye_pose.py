...
def get_response (forward_images, input_folder):
    ...
    output_folder = f"mp-eyepose-{base_name}"
...
def gaze_direction (results, img_w, img_h): 
    ...
    if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                ...
                right_eye_landmarks = [[lm.x * img_w, lm.y * img_h] for idx, lm in enumerate(face_landmarks.landmark) if idx in [33, 133]]
                left_eye_landmarks = [[lm.x * img_w, lm.y * img_h] for idx, lm in enumerate(face_landmarks.landmark) if idx in [362, 263]]
    ...
    for image_name in os.listdir(input_folder):
        if image_name.lower().endswith(('.png', '.jpg', '.jpeg')):
            ...
            if gaze:
                ...
                if (not eyes_are_closed) and (left_eye == "Center" and right_eye == "Center"): 
